  ...
  (case vm
    ...
    (apply run "clojure" "-Sdeps"
      (cond
        (= "latest" version)
        (str "{:paths [\"src\"]"
          "    :deps {datahike {:local/root \"..\"}}}")

        (re-matches #"\d+\.\d+\.\d+" version)
        (str "{:paths [\"src\"]"
          "    :deps {datahike {:mvn/version \"" version "\"}}}")

        (re-matches #"[0-9a-fA-F]{40}" version)
        (str "{:paths [\"src\"]"
          "    :deps {datahike {:git/url \"https://github.com/tonsky/datahike.git\" :sha \"" version "\"}}}"))
      "-m" "datahike-bench.datahike"
      benchmarks)
