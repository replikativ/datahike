...
#?(:cljs (declare chunk-step realize-slice lookup-ge-step avet-first-eid-step))
...
#?(:cljs
   (defn- temporal-merge-slice-step
     "Dual-mode temporal-merge-slice: realizes the current (+ temporal)
      slices — awaits in async mode — then runs the SAME pure merge pipeline
      (distinct-datoms / post-process-datoms / maybe-post-process) over the
      realized vectors. FilteredDB predicates inside maybe-post-process stay
      synchronous (documented: predicates are sync-only)."
     [origin-db from-d to-d temporal-type temporal-tx-filter db sync?]
     (async+sync sync?
                 (let [current (pca/await (realize-slice (:eavt origin-db) from-d to-d :eavt nil sync?))]
                   (case temporal-type
                     :historical
                     (let [tv (pca/await (realize-slice (:temporal-eavt origin-db) from-d to-d :eavt nil sync?))]
                       (into [] (maybe-post-process (dbu/distinct-datoms origin-db :eavt current tv) db origin-db)))

                     (:as-of :since)
                     (let [tv (pca/await (realize-slice (:temporal-eavt origin-db) from-d to-d :eavt nil sync?))
                           ctx (dbi/-search-context db)]
                       (into [] (db/post-process-datoms (dbu/distinct-datoms origin-db :eavt current tv)
                                                        origin-db ctx)))

                     ;; regular DB
                     (into [] (maybe-post-process current db origin-db)))))))

#?(:cljs
   (defn- visible-eavt-datom-step
     "Dual-mode visible-eavt-datom (as-of card-one fallback)."
     [origin-db db temporal-type temporal-eavt
      eid ra vg? vgv check-v? check-tx? scan-d temporal-tx-filter added-filter sync?]
     (async+sync sync?
                 (when temporal-eavt
                   (let [from-d (datom eid ra (when vg? vgv) tx0)
                         to-d (datom eid ra (when vg? vgv) txmax)
                         slice (pca/await (temporal-merge-slice-step origin-db from-d to-d temporal-type temporal-tx-filter db sync?))]
                     (some (fn [^Datom td]
                             (when (temporal-merge-datom-match? td eid ra vg? vgv check-v? check-tx? scan-d
                                                                temporal-tx-filter added-filter)
                               td))
...
(defn build-scan-slice-step
  "Dual-mode build-scan-slice. sync? true delegates to build-scan-slice
   unchanged (the only mode on the JVM). sync? false (cljs) returns an async
   expression yielding either a PSS AsyncSeq (plain regular-DB scans —
   streamed chunk-wise) or a REALIZED vector (temporal merges /
   post-processed paths — the same pure pipelines over awaited slice
   realizations; chunk-step's async arm serves realized colls through
   immediately-resolving pseudo-chunks)."
  [db db-index from-datom to-datom index temporal origin-db resolved-a sync?]
  #?(:clj
     (do (when (false? sync?)
           (throw (ex-info "async scan acquisition is not supported on the JVM"
                           {:error :storage/async-unsupported})))
         (build-scan-slice db db-index from-datom to-datom index temporal origin-db resolved-a))
     :cljs
     (if sync?
       (build-scan-slice db db-index from-datom to-datom index temporal origin-db resolved-a)
       (async+sync sync?
                   (if-not temporal
                     ...
                     (let [temporal-type (:type temporal)
                       ...
                       (case temporal-type
                         :historical
                         (let [cur (pca/await (realize-slice db-index from-datom to-datom index nil sync?))
                               tv (pca/await (realize-slice (get origin-db temporal-index-key) from-datom to-datom index nil sync?))]
                           (into [] (maybe-post-process (dbu/distinct-datoms origin-db index cur tv) db origin-db)))
                         ...
                         (if as-of-at-max-tx?
                           ...
                           (let [cur (pca/await (realize-slice db-index from-datom to-datom index nil sync?))
                                 tv (pca/await (realize-slice (get origin-db temporal-index-key) from-datom to-datom index nil sync?))
                                 ctx (dbi/-search-context db)]
                             (into [] (db/post-process-datoms (dbu/distinct-datoms origin-db index cur tv)
                                                              origin-db ctx))))
   ...
   (defn- chunk-step
     ...
     (if sync?
       (when s
         (when-let [sq (seq s)]
           (if (chunked-seq? sq)
             ;; PSS Iter / cljs ArrayChunk: [arr off end] fields
             (let [ch (chunk-first sq)]
               [(.-arr ch) (.-off ch) (.-end ch) (chunk-next sq)])
             ;; generic seqs (e.g. temporal merged slices, filtered seqs):
             ;; element-wise pseudo-chunks — matches the old doseq behavior
             [(array (first sq)) 0 1 (next sq)])))
       (cond
         (nil? s) (async nil)
         ;; realized colls (temporal merged vectors, filtered seqs) — serve
         ;; sync pseudo-chunks through an immediately-resolving async expr
         (not (instance? btset/AsyncSeq s))
         (let [step (chunk-step s true)] (async step))
...
(defn- execute-temporal-merge
  ...
  [db eavt-pss slice ground-filter strict-filter
   ...
   collect-set collect-datom-field collect-merge-idx
   merge-datoms n-find find-source const-vals
   result-list max-n n-merges temporal-ctx cancel sync?]
         ...
         (while (and (.hasNext iter)
           ...
           (let [^Datom scan-d (.next iter)]
             (when (and (scan-filter-temporal scan-d ground-filter strict-filter probe-set probe-datom-field temporal-tx-filter)
                        (or (nil? scan-added-val) (= (datom/datom-added scan-d) scan-added-val)))
               (let [eid (.-e scan-d)
                     ;; PHASE 1 — realize per-level match lists (see
                     ;; execute-card-many-merge): levels depend only on the
                     ;; scan datom + level constants. nil level = anti-pass;
                     ;; nil result = scan datom rejected. The temporal cursor
                     ;; fast path keeps its per-level memo semantics: the
                     ;; merge-datoms slot doubles as an eid-keyed cache and is
                     ;; written here exactly as the recursion wrote it.
                     levels
                     (loop [mi 0 acc []]
                       (if (>= mi n-merges)
                         acc
                             ...
                             (let [temporal-only? (aget merge-temporal-only mi)]
                               (if (and temporal-only? temporal-cursors (aget temporal-cursors mi)
                                 ...
                                 (let [^longs cache-eid-arr (aget merge-cursor-cache mi)
                                   ...
                                   (if (== cached-eid (long eid))
                                     (let [cached (aget merge-datoms mi)]
                                       (when (nil? cached) (recur (inc mi) (conj acc nil))))
                                     (let [probe (datom eid ra (when vg? vgv) tx0)
                                       ...
                                       (let [check-v? (aget merge-check-scan-v mi)
                                         ...
                                         (when (nil? match) (recur (inc mi) (conj acc nil)))))))
                                 ...
                                 (let [from-d (datom eid ra (when vg? vgv) tx0)
                                       ...
                                       matches (into []
                                                     (filter (fn [^Datom d]
                                                               (temporal-merge-datom-match? d eid ra vg? vgv check-v? check-tx? scan-d temporal-tx-filter added-filter)))
                                                     mslice)]
                                   (if anti?
                                     (when (empty? matches) (recur (inc mi) (conj acc nil)))
                                     (cond
                                       (seq matches) (recur (inc mi) (conj acc matches))
                                       ;; Optional merge (get-else): default-valued datom when
                                       ;; no version matched (:historical forces card-many, so
                                       ;; a card-one get-else lands here).
                                       (and merge-optional (aget merge-optional mi))
                                       (recur (inc mi) (conj acc [(datom eid ra (aget merge-defaults mi) tx0)]))
                                       :else nil)))))
                             ;; Card-one merge
                                   ...
                                   (if found?
                                     ...
                                     (if-let [found-t (when (= temporal-type :as-of)
                                                        (visible-eavt-datom origin-db db temporal-type (:temporal-eavt origin-db)
                                                                            ...
                                                                            temporal-tx-filter added-filter))]
                                       (when-not anti?
                                         (recur (inc mi) (conj acc [found-t])))
                                       (cond
                                         anti? (recur (inc mi) (conj acc nil))
                                         (and merge-optional (aget merge-optional mi))
                                         (recur (inc mi) (conj acc [(datom eid ra (aget merge-defaults mi) tx0)]))
                                         :else nil))))
                           ...
                           (when (and (< si send)
                             ...
                             (let [scan-d (aget skeys si)]
                               (when (and (scan-filter-temporal scan-d ground-filter strict-filter probe-set probe-datom-field temporal-tx-filter)
                                          (or (nil? scan-added-val) (= (datom/datom-added scan-d) scan-added-val)))
                                 (let [eid (.-e ^Datom scan-d)
                                       ;; PHASE 1 — realize per-level match lists (see the :clj arm).
                                       levels
                                       (loop [mi 0 acc []]
                                         (if (>= mi n-merges)
                                           acc
                                           (let [anti? (aget merge-anti mi)
                                             ...
                                             (if card-many?
                                               (let [from-d (datom eid ra (when vg? vgv) tx0)
                                                     ...
                                                     match? (fn [^Datom d]
                                                              (temporal-merge-datom-match? d eid ra vg? vgv check-v? check-tx? scan-d temporal-tx-filter added-filter))
                                                     matches (if temporal-only?
                                                               (pca/await (realize-slice temporal-eavt-pss from-d to-d :eavt match? sync?))
                                                               (into [] (filter match?)
                                                                     (pca/await (temporal-merge-slice-step origin-db from-d to-d temporal-type temporal-tx-filter db sync?))))]
                                                 (if anti?
                                                   (when (empty? matches) (recur (inc mi) (conj acc nil)))
                                                   (cond
                                                     (seq matches) (recur (inc mi) (conj acc matches))
                                                     ;; Optional merge (get-else): default when no version
                                                     ;; matched (:historical forces card-many).
                                                     (and merge-optional (aget merge-optional mi))
                                                     (recur (inc mi) (conj acc [(datom eid ra (aget merge-defaults mi) tx0)]))
                                                     :else nil)))
                                               (let [probe (datom eid ra vgv tx0)
                                                     ...
                                                     found-d (or (when (and d (temporal-merge-datom-match? d eid ra vg? vgv check-v? check-tx? scan-d temporal-tx-filter added-filter))
                                                                 ...
                                                                 (when (= temporal-type :as-of)
                                                                   (pca/await (visible-eavt-datom-step origin-db db temporal-type (:temporal-eavt origin-db)
                                                                                                       eid ra vg? vgv check-v? check-tx? scan-d
                                                                                                       temporal-tx-filter added-filter sync?))))]
                                                 (if anti?
                                                   (when (not found-d) (recur (inc mi) (conj acc nil)))
                                                   (cond
                                                     found-d (recur (inc mi) (conj acc [found-d]))
                                                     ;; Optional merge: synthetic default datom on miss
                                                     (and merge-optional (aget merge-optional mi))
                                                     (recur (inc mi) (conj acc [(datom eid ra (aget merge-defaults mi) tx0)]))
                                                     :else nil)))))))]
                                   ;; PHASE 2 — odometer, deepest level fastest.
...
(defn- execute-group-direct
  ...
  [db scan-op merge-ops find-vars consts
   ...
   collect-set collect-datom-field collect-merge-idx
   max-results
   & {:keys [scan-estimate pipeline temporal cancel sync? slice-override]
      :or {sync? true}}]
  ;; ONE dual body: on :clj the macro emits only the stripped synchronous
  ;; form; on :cljs sync? dispatches at runtime. The two awaits below
  ;; (slice acquisition, kernel expression) strip away in sync mode.
  (async+sync sync?
                                ...
                                (if sync?
                                  (if temporal
                                    (build-scan-slice db db-index from-datom to-datom index
                                                      temporal index-db resolved-a)
                                    (di/-slice db-index from-datom to-datom index))
                                  (pca/await (build-scan-slice-step db db-index from-datom to-datom index
                                                                    temporal index-db resolved-a sync?)))))
