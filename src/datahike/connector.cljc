...
(extend-protocol IConfiguration
    ...
    (let [config (dc/load-config config)
          ...
          _ (when-not stored-db
              ...
              (dt/raise "Database does not exist." {:type :db-does-not-exist
                                                    :config config}))
          {:keys [eavt-key aevt-key avet-key temporal-eavt-key temporal-aevt-key temporal-avet-key schema rschema config max-tx op-count hash meta]
  ...
  (-create-database [config & deprecated-config]
    (let [{:keys [keep-history? initial-tx] :as config} (dc/load-config config deprecated-config)
          ...
          {:keys [eavt aevt avet temporal-eavt temporal-aevt temporal-avet schema rschema config max-tx op-count hash meta]}
