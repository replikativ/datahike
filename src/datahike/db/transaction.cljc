(ns datahike.db.transaction
  (:require #?(:cljs [datahike.db :refer [HistoricalDB]])
            [clojure.set :as set]
            [clojure.spec.alpha :as s]
            [clojure.string :as str] 
            [datahike.constants :refer [tx0]]
            [datahike.datom :as dd :refer [datom datom-added datom-tx datom? #?(:cljs Datom)]]
...
(defn update-schema [db ^Datom datom]
  (let [schema (dbi/-schema db)
    ...
    (when (and attribute-refs? (contains? (dbi/-system-entities db) e))
      (dt/raise "System schema entity cannot be changed"
             {:error :transact/schema :entity-id e}))
    (if (= a-ident :db/ident)
      (if (schema v-ident)
        (dt/raise (str "Schema with attribute " v-ident " already exists")
  ...
  (let [schema (dbi/-schema db)
    ...
    (if (= a-ident :db/ident)
      (if-not (schema v-ident)
        (dt/raise (str "Schema with attribute " v-ident " does not exist")
      ...
      (if-let [schema-entry (schema e)]
        ...
        (dt/raise (str "Schema with entity id " e " does not exist")
  ...
  (when (dbu/is-attr? db (.-a datom) :db/unique)
    (when-let [old (first (dbi/-datoms db :avet [(.-a datom) (.-v datom)]))]
      (when-not (= (.-e datom) (.-e old))
        (dt/raise "Cannot add " datom " because of unique constraint: " old
...
(defn- check-upsert-conflict [entity acc]
  (let [[e a v] acc
    ...
    (if (or (nil? _e)
            ...
            (== _e e))
      acc
      (dt/raise "Conflicting upsert: " [a v] " resolves to " e
          ...
          (let [a (if (:attribute-refs? (dbi/-config db))
            ...
            (if-some [e (when v
              ...
              (cond
                ...
                :else
                (let [[_e _a _v] acc]
                  (dt/raise "Conflicting upserts: " [_a _v] " resolves to " _e
...
(defn- explode [db entity]
  (let [eid (:db/id entity)
        ...
        _ (when (and attribute-refs? (contains? (dbi/-system-entities db) eid))
            (dt/raise "Entity with ID " eid " is a system attribute " (dbi/-ident-for db eid) " and cannot be changed"
        ...
        entities (for [[a-ident vs] entity
                       ...
                       :let [_ (dbu/validate-attr-ident a-ident {:db/id eid, a-ident vs} db)
                             ...
                             _ (when (and reverse? (not (dbu/ref? db straight-a-ident)))
                                 (dt/raise "Bad attribute " a-ident ": reverse attribute name requires {:db/valueType :db.type/ref} in schema"
...
(defn- retry-with-tempid [initial-report report es tempid upserted-eid]
  (if (contains? (:tempids initial-report) tempid)
    (dt/raise "Conflicting upsert: " tempid " resolves"
         ...
         (if (some? straight-a)
           (conj entities
                 [:db/add
                  ...
                  value
                  tid])
           (dt/raise "Bad transaction meta attribute " attribute " at " tx-meta ", not defined in system or current schema"
...
(defn check-schema-update [db entity new-eid]
  (when (ds/schema-entity? entity)
    (when (and (contains? entity :db/ident)
      ...
      (dt/raise "Using namespace 'db' for attribute identifiers is not allowed"
    ...
    (if-let [attr-name (get-in db [:schema new-eid])]
      (when-let [invalid-updates (ds/find-invalid-schema-updates entity (get-in db [:schema attr-name]))]
        (when-not (empty? invalid-updates)
          (dt/raise "Update not supported for these schema attributes"
      ...
      (when (= :write (get-in db [:config :schema-flexibility]))
        (when (or (:db/cardinality entity) (:db/valueType entity))
          (when-not (ds/schema? entity)
            (dt/raise "Incomplete schema transaction attributes, expected :db/ident, :db/valueType, :db/cardinality"
                   {:error :transact/schema :entity entity})))))))
  ...
  (let [old-eid (:db/id entity)
    ...
    (if (and (some? upserted-eid)
      ...
      (let [new-eid (cond
                      ...
                      :else (dt/raise "Expected number, string or lookup ref for :db/id, got " old-eid
  ...
  (let [[_ e a ov nv] op-vec
    ...
    (if (nil? ov)
      (if (empty? datoms)
        ...
        (dt/raise ":db.fn/cas failed on datom [" e " " a " " (if (dbu/multival? db a) (map :v datoms) (:v (first datoms))) "], expected nil"
      ...
      (let [ov (if (dbu/ref? db a) (dbu/entid-strict db ov) ov)]
        ...
        (if (dbu/multival? db a)
          (if (some (fn [^Datom d] (= (.-v d) ov)) datoms)
            [(transact-add report [:db/add e a nv]) []]
            (dt/raise ":db.fn/cas failed on datom [" e " " a " " (map :v datoms) "], expected " ov
                   {:error :transact/cas, :old datoms, :expected ov, :new nv}))
          (let [v (:v (first datoms))]
            (if (= v ov)
              [(transact-add report [:db/add e a nv]) []]
              (dt/raise ":db.fn/cas failed on datom [" e " " a " " v "], expected " ov
...
(defn check-tuple [db op-vec]
  (let [[_ _ a v] op-vec
    ...
    (cond (:db/tupleType attr-schema)
          (cond (> (count v) 8)
                (dt/raise "Cannot store more than 8 values for homogeneous tuple: " op-vec
                       {:error :transact/syntax, :tx-data op-vec})

                (not (apply = (map type v)))
                (dt/raise "Cannot store homogeneous tuple with values of different type: " op-vec
                       {:error :transact/syntax, :tx-data op-vec})

                (not (s/valid? (-> db dbi/-schema a :db/tupleType) (first v)))
                (dt/raise "Cannot store homogeneous tuple. Values are of wrong type: " op-vec
                       {:error :transact/syntax, :tx-data op-vec}))
          (:db/tupleTypes attr-schema)
          (cond (not (= (count v) (count (:db/tupleTypes attr-schema))))
                (dt/raise (str "Cannot store heterogeneous tuple: expecting " (count (:db/tupleTypes attr-schema)) " values, got " (count v))
                       {:error :transact/syntax, :tx-data op-vec})

                (not (apply = (map s/valid? (:db/tupleTypes attr-schema) v)))
                (dt/raise (str "Cannot store heterogeneous tuple: there is a mismatch between values " v " and their types " (:db/tupleTypes attr-schema))
                       {:error :transact/syntax, :tx-data op-vec}))
          (and (:db/tupleAttrs attr-schema)
               (not (::internal (meta op-vec))))
          (dt/raise "Can’t modify tuple attrs directly: " op-vec
                 {:error :transact/syntax, :tx-data op-vec}))))
  ...
  (let [[op e a v] op-vec]
    (case op
      ...
      :db/purge (if (dbi/-keep-history? db)
                  (let [history (HistoricalDB. db)]
                    (if-some [e (dbu/entid history e)]
                      ...
                      (dt/raise "Can't find entity with ID " e " to be purged"
                             {:error :transact/purge, :operation op, :tx-data op-vec})))
                  (dt/raise "Purge is only available in temporal databases."
                         {:error :transact/purge :operation op :tx-data op-vec}))
      ...
      :db.purge/attribute (if (dbi/-keep-history? db)
                            (let [history (HistoricalDB. db)]
                              (if-let [e (dbu/entid history e)]
                                ...
                                (dt/raise "Can't find entity with ID " e " to be purged"
                                       {:error :transact/purge, :operation op, :tx-data op-vec})))
                            (dt/raise "Purge attribute is only available in temporal databases."
                                   {:error :transact/purge :operation op :tx-data op-vec}))
      ...
      :db.purge/entity (if (dbi/-keep-history? db)
                         (let [history (HistoricalDB. db)]
                           (if-let [e (dbu/entid history e)]
                             ...
                             (dt/raise "Can't find entity with ID " e " to be purged"
                                    {:error :transact/purge, :operation op, :tx-data op-vec})))
                         (dt/raise "Purge entity is only available in temporal databases."
                                {:error :transact/purge :operation op :tx-data op-vec}))
      ...
      :db.ensure/attrs (let [{:keys [tx-data]} report
                         ...
                         (if (empty? diff)
                           [report []]
                           (dt/raise "Entity " e " missing attributes " diff " of spec " a
      ...
      :db.ensure/preds (let [{:keys [db-after]} report
                         ...
                         (if-not (empty? preds)
                           (dt/raise "Entity " e " failed predicates " preds " of spec " a
                                  {:error :transact/ensure :operation op :tx-data op-vec})
                           [report []]))
        ...
        (if-some [ident (dbu/entid db op)]
          (let [fun (-> (dbi/-search db [ident :db/fn]) first :v)
            ...
            (if (fn? fun)
              ...
              (dt/raise "Entity " op " expected to have :db/fn attribute with fn? value"
                     {:error :transact/syntax, :operation :db.fn/call, :tx-data op-vec})))
          (dt/raise "Can’t find entity for transaction fn " op
                 {:error :transact/syntax, :operation :db.fn/call, :tx-data op-vec}))
        (dt/raise (str "Unknown operation at " op-vec ", expected " (str/join "," builtin-op?)
...
(defn transact-tx-data [{:keys [db-before] :as initial-report} initial-es]
  (when-not (or (nil? initial-es)
    ...
    (dt/raise "Bad transaction data " initial-es ", expected sequential collection"
        ...
        (cond
          ...
          (let [[op e a v] entity]
            ...
            (cond
              ...
              (tempid? e)
              (if (not= op :db/add)
                (dt/raise "Can't use tempid in '" entity "'. Tempids are allowed in :db/add only"
          ...
          :else
          (dt/raise "Bad entity type at " entity ", expected map or vector"
  ...
  (loop [report (update initial-report :db-after transient)
    ...
    (let [[entity & entities] es
          ...
          a (if (:attribute-refs? config)
              (dbi/-ref-for db a-ident)
              (if (number? a)
                (dt/raise "Configuration mismatch: import data with attribute references can not be imported into a database with no attribute references."
