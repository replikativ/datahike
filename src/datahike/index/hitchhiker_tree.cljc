(ns datahike.index.hitchhiker-tree
  (:require [hitchhiker.tree.utils.async :as async]
            ...
            [datahike.constants :refer [s0 u0 e0 tx0 emax txmax]]
...
(defn- from-datom [^Datom datom index-type]
  (let [datom-seq (case index-type
    ...
    (->> datom-seq
         (remove #{e0 s0 u0 tx0 emax txmax})
         (remove nil?)
         vec)))
...
(defn -seq [tree index-type]
  (-slice tree (dd/datom s0 nil nil tx0) (dd/datom emax nil nil txmax) index-type))
