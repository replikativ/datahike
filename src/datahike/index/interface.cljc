...
(defprotocol IIndex
  ...
  (-insert [index datom index-type op-count] [index datom index-type op-count opts] "Inserts a datom into the index. Opts arity as in -slice: {:sync? false} (cljs) returns an async expression yielding the new index — the tree walk awaits lazily-loaded nodes.")
  (-temporal-insert [index datom index-type op-count] [index datom index-type op-count opts] "Inserts a datom in a history index. Opts arity as in -insert.")
  (-upsert [index datom index-type op-count old-datom] [index datom index-type op-count old-datom opts] "Inserts or updates a datom into the index. Opts arity as in -insert.")
  (-temporal-upsert [index datom index-type op-count old-datom] [index datom index-type op-count old-datom opts] "Inserts or updates a datom in a history index. Opts arity as in -insert.")
  (-remove [index datom index-type op-count] [index datom index-type op-count opts] "Removes a datom from the index. Opts arity as in -insert.")
