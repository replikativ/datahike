...
(defn current-values-from-entity-attribute-groups [db]
  (mapcat
   (fn [[[_ a] datoms]]
...
(defn- entity-attribute-pair [^Datom datom]
  [(.-e datom) (.-a datom)])
  ...
  ([datoms time-point db final-xform]
   (let [as-of-pred (fn [^Datom d]
         ...
         filtered-datoms (->> datoms
                              ...
                              (group-by entity-attribute-pair)
                              (into [] (comp (current-values-from-entity-attribute-groups db)
                                             final-xform)))]
     filtered-datoms)))
