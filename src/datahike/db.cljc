...
(defn- with-datom-upsert [db ^Datom datom]
  ...
  (let [indexing?     (indexing? db (.-a datom))
    ...
    (cond-> db
      ...
      keep-history? (update-in [:temporal-eavt] #(di/-temporal-upsert % datom :eavt op-count (:v old-datom)))
      true          (update-in [:eavt] #(di/-upsert % datom :eavt op-count old-datom))

      keep-history? (update-in [:temporal-aevt] #(di/-temporal-upsert % datom :aevt op-count (:v old-datom)))
      true          (update-in [:aevt] #(di/-upsert % datom :aevt op-count old-datom))

      (and keep-history? indexing?) (update-in [:temporal-avet] #(di/-temporal-upsert % datom :avet op-count (:v old-datom)))
