                ...
                (let [db  (-> (d/empty-db {:aka {:db/cardinality :db.cardinality/many}})
                    ...
                    (testing "Retract"
                      (let [db  (-> db
                        ...
                        (is (= (into {} (<! (d/touch (<! (d/entity db 1))))) {:aka #{"Tupen"}}))))
              ...
              (go
                (let [conn    (<! (d/create-conn {:name {:db/unique :db.unique/identity}}))
                  ...
                  (d/transact! conn [{:db/id    1
                                          ...
                                          {:db/ident :inc-age
                                           :db/fn    inc-age}])
                  #_(is (thrown-msg? "Can’t find entity for transaction fn :unknown-fn"
                                     (d/transact! conn [[:unknown-fn]])))
                  #_(is (thrown-msg? "Entity :Petr expected to have :db/fn attribute with fn? value"
                  ...
                  (testing tx-tempid
                    (let [conn (<! (d/create-conn {:created-at {:db/valueType :db.type/ref}}))
                      ...
                      (let [tx2   (<! (d/transact! conn [[:db/add tx-tempid :prop3 "prop3"]]))
                        ...
                        (is (= (into {} (<! (d/touch (<! (d/entity @conn tx-id)))))
                               {:prop3 "prop3"})))
