...
(deftest test-basics
  #?(:cljs 
     (t/async done
              (go (are [find res] (= (set (<! (d/q {:find  find
                                                    :where '[[?e :age ?a]
                                                             [(>= ?a 18)]]}
                                                   (<! (test-db)))))
                                     res)
                  ...
                  (done)))
     :clj (are [find res] (= (set (d/q {:find find
                                        :where '[[?e :age ?a]
                                                 [(>= ?a 18)]]}
                                       test-db))
                             res)
...
(deftest test-var-pattern
  #?(:cljs 
     (t/async done 
              (go (are [find pattern res] (= (set (<! (d/q {:find find
                                                            :in   '[$ ?pattern]
                                                            :where '[[?e :age ?a]
                                                                     [(>= ?a 18)]]}
                                                           (<! (test-db)) pattern)))
                  ...
                  (done)))
     :clj (are [find pattern res] (= (set (d/q {:find find
                                                :in   '[$ ?pattern]
                                                :where '[[?e :age ?a]
                                                         [(>= ?a 18)]]}
                                               test-db pattern))
              ...
              (go (let [db1 (<! (d/db-with (<! (d/empty-db)) [{:db/id 1 :name "Ivan" :age 25}]))
                    ...
                    #_(testing "$ is default source"
                      (is (= (set (<! (d/q '[:find ?e (pull ?e [:name])
                             ...
                             #{[1 {:name "Petr"}]}))))
                  (done)))
     :clj (let [db1 (d/db-with (d/empty-db) [{:db/id 1 :name "Ivan" :age 25}])
                db2 (d/db-with (d/empty-db) [{:db/id 1 :name "Petr" :age 25}])]
            (is (= (set (d/q '[:find ?e (pull $1 ?e [:name])
                               :in $1 $2
                               :where [$1 ?e :age 25]]
...
(deftest test-lookup-refs
  #?(:cljs 
     (t/async done
              (go (let [db (<! (d/db-with (<! (d/empty-db {:name {:db/unique :db.unique/identity}}))
                                          [{:db/id 1 :name "Petr" :age 44}
                                           {:db/id 2 :name "Ivan" :age 25}
                                           {:db/id 3 :name "Oleg" :age 11}]))]
                  ...
                  (done)))
     :clj (let [db (d/db-with (d/empty-db {:name {:db/unique :db.unique/identity}})
                              [{:db/id 1 :name "Petr" :age 44}
                               {:db/id 2 :name "Ivan" :age 25}
                               {:db/id 3 :name "Oleg" :age 11}])]
            (is (= (set (d/q '[:find ?ref ?a (pull ?ref [:db/id :name])
