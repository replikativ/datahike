...
(deftest test-components
  ...
  (let [create-db #(d/db-with
    ...
    (testing "touch"
      (is (= (touched (d/entity (create-db) 1))
            {:db/id 1
             :name "Ivan"
             :profile {:db/id 3
                       :email "@3"}}))
      (is (= (touched (d/entity (d/db-with (create-db) [[:db/add 3 :profile 4]]) 1))
            {:db/id 1
             :name "Ivan"
             :profile {:db/id 3
                       :email "@3"
    ...
    (testing "retractEntity"
      (let [db (d/db-with (create-db) [[:db.fn/retractEntity 1]])]
    ...
    (testing "retractAttribute"
      (let [db (d/db-with (create-db) [[:db.fn/retractAttribute 1 :profile]])]
        (is (= (d/q '[:find ?a ?v :where [3 ?a ?v]] db)
              #{}))))
    
    (testing "reverse navigation"
      (is (= (visible (:_profile (d/entity (create-db) 3)))
             {:db/id 1})))))
