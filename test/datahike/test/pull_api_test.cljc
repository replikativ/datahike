...
(def test-db 
  #?(:cljs #(d/init-db test-datoms test-schema)
     :clj (d/init-db test-datoms test-schema)))
...
(deftest test-pull-as
  #?(:cljs 
     (t/async done
              (go (is (= {"Name" "Petr", :alias ["Devil" "Tupen"]}
                         (<! (d/pull (<! (test-db)) '[[:name :as "Name"] [:aka :as :alias]] 1))))
                  (done)))
     :clj (is (= {"Name" "Petr", :alias ["Devil" "Tupen"]}
                 (d/pull test-db '[[:name :as "Name"] [:aka :as :alias]] 1)))))
...
(deftest test-dual-recursion
  #?(:cljs 
     (t/async done 
              (go (let [empty (<! (d/empty-db {:part {:db/valueType :db.type/ref}
                                               :spec {:db/valueType :db.type/ref}}))
