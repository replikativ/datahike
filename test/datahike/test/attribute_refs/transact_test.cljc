...
(deftest test-db-fn-cas
  (let [conn (setup-new-connection)
    ...
    (is (thrown-with-msg? Throwable (re-pattern (str ":db.fn/cas failed on datom [" (+ ref-e0 1) " " weight-ref " 400], expected 200"))
  ...
  (let [conn (setup-new-connection)
    ...
    (is (thrown-with-msg? Throwable (re-pattern (str ":db.fn/cas failed on datom [" (+ ref-e0 1) " " label-ref " (:x :y :z)], expected :s"))
