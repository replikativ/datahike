...
(deftest test-explode-ref
  #?(:cljs 
     (t/async done
              (go (let [db0 (<! (d/empty-db {:children {:db/valueType :db.type/ref
...
(deftest test-explode-nested-maps
  #?(:cljs 
     (t/async done
              (go (do (let [schema {:profile {:db/valueType :db.type/ref}}
                  ...
                  (done)))
     :clj (do (let [schema {:profile {:db/valueType :db.type/ref}}
                    db     (d/empty-db schema)]
                (are [tx res] (= (d/q '[:find ?e ?a ?v
                                        :where [?e ?a ?v]]
                                      (d/db-with db tx)) res)
