...
(deftest test-with
  (let [init-db-fn #(-> (d/empty-db {:aka { :db/cardinality :db.cardinality/many }})
...
(deftest test-retract-fns
  (let [init-db-fn #(-> (d/empty-db {:aka    { :db/cardinality :db.cardinality/many }
    ...
    (is (= (d/db-with (init-db-fn) [[:db.fn/retractEntity 1]])
          (d/db-with (init-db-fn) [[:db/retractEntity 1]])))

   (testing "Retract entitiy with incoming refs"
      (is (= (d/q '[:find ?e :where [1 :friend ?e]] (init-db-fn))
             #{[2]}))
   ...
   (let [db (d/db-with (init-db-fn) [ [:db.fn/retractAttribute 1 :name] ])]
      (is (= (d/q '[:find ?a ?v
                    :where [1 ?a ?v]] db)
    ...
    (let [db (d/db-with (init-db-fn) [ [:db.fn/retractAttribute 1 :aka] ])]
      (is (= (d/q '[:find ?a ?v
                    :where [1 ?a ?v]] db)
...
(deftest test-db-fn-cas
  ...
  ;; TODO: there seems to be a fdb bug here.
  #_(let [conn (d/create-conn {:label { :db/cardinality :db.cardinality/many }})]
