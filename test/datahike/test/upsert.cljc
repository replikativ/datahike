...
(comment (let [edb     (d/empty-db {:name  {:db/unique :db.unique/identity} ; TODO: check empty-db with schema in tests!
               ...
               e3      (+ 3 (:max-eid edb))
               _       (print e1 e2 e3)
               db      (d/db-with edb [{:db/id e1 :name "Ivan" :email "@1"}
                                       {:db/id e2 :name "Petr" :email "@2"}])
...
(deftest test-upsert
  (let [edb (d/empty-db {:name  {:db/unique :db.unique/identity} ; TODO: check empty-db with schema in tests!
    ...
    #_(testing "upsert and :current-tx conflict"            ;; TODO: check if different then randomized
