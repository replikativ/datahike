...
(deftest test-predicate-assertion
  (let [schema (conj schema-template
                     {:db/ident :account/guard
                      :db.entity/preds ['datahike.test.entity-spec/is-email? 'datahike.test.entity-spec/positive-balance?]})
    ...
    (letfn [(tx-with-ensure [account]
      ...
      (testing "assert invalid account with one invalid predicate"
        (is (thrown-msg?
             "Entity 5 failed predicates #{datahike.test.entity-spec/is-email?} of spec :account/guard"
             (tx-with-ensure invalid-account))))
      (testing "assert invalid account with mulitple invalid predicates"
        (is (thrown-msg?
             "Entity 5 failed predicates #{datahike.test.entity-spec/positive-balance? datahike.test.entity-spec/is-email?} of spec :account/guard"
