...
(deftest test-or
  #?(:cljs
    ...
    ;; OR introduces vars in different order
                  ...
                  #{1 5 4})
                (done)))
     :clj (are [q res] (= (d/q (concat '[:find ?e :where] (quote q)) @test-db)
                          (into #{} (map vector) res))
...
(deftest test-or-join
  #?(:clj
     (do (are [q res] (= (d/q (concat '[:find ?e :where] (quote q)) (<! (test-db)))
                         (into #{} (map vector) res))
           [(or-join [?e]
                     [?e :name ?n]
                     (and [?e :age ?a]
...
(deftest test-errors
  #?(:clj (do (is (thrown-msg? "Join variable not declared inside clauses: [?a]"
                               (d/q '[:find ?e
                                      :where (or [?e :name _]
                                                 [?e :age ?a])]
                                    @test-db)))
