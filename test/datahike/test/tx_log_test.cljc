...
(def complete-result [{:tx 1
                      ...
                      {:tx 2
                       :data
                       [[4 :name "Anne" (+ tx1 1) true]
                        [4 :age 25 (+ tx1 1) true]
                        [5 :name "Bernard" (+ tx1 1) true]
                        [5 :age 30 (+ tx1 1) true]]}
                      {:tx 3
                       :data
                       [[6 :name "Catherine" (+ tx1 2) true]
                        [6 :age 5 (+ tx1 2) true]
...
(defn setup-data [conn]
  (let [schema [{:db/ident       :name
    ...
    (d/transact conn {:tx-data [{:name "Anne"
                                 :age  25}
                                {:name "Bernard"
                                 :age 30}]})

    (d/transact conn {:tx-data [{:name    "Catherine"
                                 :age     5
                                 :parents [[:name "Anne"]
                                           [:name "Bernard"]]}]})))

(deftest test-tx-log-without-attr-refs
  (let [conn        (u/setup-db {:store              {:backend :mem
                                                      :id      "test-tx-range"}
                                 :keep-history?      true
                                 :keep-log?          true
    ...
    (testing "tx-range"
      ...
      (testing "after new transactions"
        (d/transact conn {:tx-data [[:db/add [:name "Anne"] :age 26]]})
        (d/transact conn {:tx-data [[:db/retractEntity [:name "Catherine"]]]})
        (is (= complete-result
               (cleanup-txs (d/tx-range @conn)))))
    ...
    (testing "purge"
      (let [david-entity {:name    "David"
                          :age     3
                          :parents [[:name "Anne"] [:name "Bernard"]]}
            david-datoms [[7 :name "David" 536870918 true]
                          [7 :age 3 536870918 true]
                          [7 :parents 4 536870918 true]
                          [7 :parents 5 536870918 true]]]
        (d/transact conn {:tx-data [david-entity]})
        (is (= david-datoms
               (-> @conn
                   (d/get-tx (+ tx1 5))
                   clean-tx)))

        (d/transact conn [[:db/purge [:name "David"] :age 2]])
        (is (= david-datoms
               (-> @conn
                   (d/get-tx (+ tx1 5))
                   clean-tx)))

        (d/transact conn [[:db/purge [:name "David"] :age 3]])
        (is (= [[7 :name "David" 536870918 true]
                [7 :parents 4 536870918 true]
                [7 :parents 5 536870918 true]]
               (-> @conn
                   (d/get-tx (+ tx1 5))
        ...
        (testing "attribute"
          (d/transact conn [[:db.purge/attribute [:name "David"] :parents]])
          (is (= [[7 :name "David" 536870918 true]]
                 (-> @conn
                     (d/get-tx (+ tx1 5))
                     clean-tx))))
        ...
        (testing "entity"
          (d/transact conn [[:db.purge/entity [:name "David"]]])
          (is (= []
                 (-> @conn
                     (d/get-tx (+ tx1 5))
                     clean-tx))))))))
...
(def attr-ref-result [{:tx 0
                      ...
                      {:tx 2,
                       :data
                       [[42 :name "Anne" (+ tx1 1) true]
                        [42 :age 25 (+ tx1 1) true]
                        [43 :name "Bernard" (+ tx1 1) true]
                        [43 :age 30 (+ tx1 1) true]]}
                      {:tx 3,
                       :data
                       [[44 :name "Catherine" (+ tx1 2) true]
                        [44 :age 5 (+ tx1 2) true]
  ...
  (let [conn            (u/setup-db {:store              {:backend :mem
    ...
    (testing "tx-range"
      (testing "after setup"
        (is (= (take 4 complete-result)
               (cleanup-txs (d/tx-range @conn)))))
      (testing "after new transactions"
        (d/transact conn {:tx-data [{:db/id [:name "Anne"] :age 26}]})
        (d/transact conn {:tx-data [[:db/retractEntity [:name "Catherine"]]]})
        (is (= complete-result
               (cleanup-txs (d/tx-range @conn)))))
    ...
    (testing "purge"
      (let [david-entity {:name    "David"
                          :age     3
                          :parents [[:name "Anne"] [:name "Bernard"]]}
            david-datoms [[45 :name "David" 536870918 true]
        ...
        (d/transact conn [[:db/purge [:name "David"] 41 2]])
        (is (= david-datoms
               (-> @conn
                   (d/get-tx (+ tx1 5))
                   clean-tx)))

        (d/transact conn [[:db/purge [:name "David"] 41 3]])
        (is (= [[45 :name "David" 536870918 true]
                [45 :parents 42 536870918 true]
                [45 :parents 43 536870918 true]]
               (-> @conn
                   (d/get-tx (+ tx1 5))
        ...
        (testing "attribute"
          (d/transact conn [[:db.purge/attribute [:name "David"] 40]])
          (is (= [[45 :name "David" 536870918 true]]
                 (-> @conn
                     (d/get-tx (+ tx1 5))
                     clean-tx))))
