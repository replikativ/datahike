  ...
  #?(:cljs 
     (t/async done
      ...
      ;; Retractions
                  ...
                  #_(are [tx msg] (thrown-msg? msg (d/db-with db tx))
...
#_(deftest test-lookup-refs-transact-multi
  #?(:clj (let [db (d/db-with (d/empty-db {:name    {:db/unique :db.unique/identity}
...
#_(deftest lookup-refs-index-access
  #?(:clj (let [db (d/db-with (d/empty-db {:name    {:db/unique :db.unique/identity}
                                           :friends {:db/valueType :db.type/ref
                                                     :db/cardinality :db.cardinality/many}})
...
#_(deftest test-lookup-refs-query
  #?(:clj (let [schema {:name   {:db/unique :db.unique/identity}
                        :friend {:db/valueType :db.type/ref}}
