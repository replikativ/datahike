...
(defn test-store [cfg]
  #?(:cljs 
     (go (let [_ (<! (d/delete-database cfg))]
     ...
     :clj (let [_ (d/delete-database cfg)]
            (is (not (d/database-exists? cfg)))
            (let [_ (d/create-database (merge cfg {:schema-flexibility :read}))
                  conn (d/connect cfg)]
              (d/transact conn [{:db/id 1, :name  "Ivan", :age   15}
   ...
   #?(:clj (deftest test-db-file-store
...
(deftest test-persistent-set-index
  #?(:cljs 
     (t/async done 
              (go 
                (is (= 1 1))
                (let [config {:store {:backend :indexeddb
                    ...
                    #_(let [conn (<! (d/connect config))]
                      (<! (d/transact conn [{:db/id 1, :name "Alice"}]))
                      #_(is (= me.tonsky.persistent_sorted_set.PersistentSortedSet
                             (-> @conn :eavt (<!) type)))))
                  (done)))
     :clj (let [config {:store {:backend :mem
                                :id "test-persistent-set"}
                        :schema-flexibility :read
                        :keep-history? false
                        :index :datahike.index/persistent-set}]
