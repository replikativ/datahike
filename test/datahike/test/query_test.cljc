...
(deftest test-bindings
  #?(:cljs
     ...
     (let [db (-> (d/empty-db)
       ...
       (testing "Error reporting"
         (is (thrown-with-msg? js/Error #"Cannot bind value :a to tuple \[\?a \?b\]")
             (<! (d/q '[:find ?a ?b :in [?a ?b]] :a)))
         (is (thrown-with-msg? js/Error #"Cannot bind value :a to collection \[\?a \.\.\.\]"
                               (d/q '[:find ?a :in [?a ...]] :a)))
         (is (thrown-with-msg? js/Error #"Not enough elements in a collection \[:a\] to bind tuple \[\?a \?b\]"
                               (d/q '[:find ?a ?b :in [?a ?b]] [:a])))))))
