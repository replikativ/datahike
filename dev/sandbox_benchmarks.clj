...
;(b/-main)                                                   ;; LOG_LEVEL=':fatal' clj -M:benchmark

;(b/-main "-t" "test-id")                                    ;; LOG_LEVEL=':fatal' clj -M:benchmark  -t test-id

;(b/-main "-t" "test-id" "-t" "test-id2")                    ;; LOG_LEVEL=':fatal' clj -M:benchmark -t test-id -t test-id2

;(b/-main "-t" "test-id" "-u" "http://localhost:3001" "-n" "benchmarks" "-g" "test-token") ;; LOG_LEVEL=':fatal' clj -M:benchmark run -o remote-db -t test-id -u http://localhost:3001 -n benchmarks -g test-token
