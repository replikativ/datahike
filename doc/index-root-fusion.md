...
*Status: implemented (EXPERIMENTAL, opt-in via `:fuse-index-roots?`). See
"As implemented" at the end for the deltas from this original design note.*
...
Option A, with these deltas from the plan above:

- **Record layout:** the inlined roots are sibling keys on the db-record
  (`:eavt-root`/`:aevt-root`/`:avet-root` + temporal), not a `:root` inside the
  index-ref — the canonical PSS root handler stays untouched and legacy records
  are distinguished by key presence.
- **Protocol:** `-root-node` / `-seed-root!` on `IIndex` (implemented for the
  persistent-set index; fusion is gated to `:datahike.index/persistent-set`).
  `stored->db`, GC's reachability walk, and the audit deep-walk all seed the
  inlined root before touching the index.
- **Caveat 1 (crypto audit) resolved:** `-recompute-merkle-root` falls back to
  verifying the seeded in-memory root (`walk-pss-node!`) when no object exists
  at the root address, then recurses into the separately-stored children.
- **Caveat: the PUT exclusion is gated OFF under `:crypto-hash?`.** Content
  -derived addresses dedup across trees, so a root's address can also be
  referenced as an interior *child* of another index's tree (e.g. a temporal
  leaf identical to the current index's whole single-leaf root); excluding the
  object would dangle that reference. squuid addresses are minted uniquely per
  stored node, so the exclusion is exact there. Under crypto, fusion still
  saves the per-index cold-open GET (root rides in the record), just not the
  PUT.
- **Connect:** `:fuse-index-roots?` is adopted from the stored config at
  connect (with `:index-config`'s create-time-fixed keys), so reconnects don't
  re-specify it.
- **First record:** `-create-database*` writes the initial record unfused;
  restore is presence-based so mixed histories are fine.
- **`versioning/branch!`** drains pending-writes unfiltered: with fusion on, a
  branch-creation record carries inlined roots *and* writes them separately —
  harmless duplication on a rare operation.
