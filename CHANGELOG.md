...
- **Attribute-value constraints** — opt-in per-attribute value validation. `:db/maxLength` bounds a string attribute's length (declarative, cljs-safe); `:db.attr/preds` names value predicates by symbol (like `:db.entity/preds`), each resolved via a process-local registry (`datahike.attr-preds/register-attr-pred!` — the cross-platform path) or, on clj, a `requiring-resolve`-able var (Datomic-style). Predicates are `(fn [value] -> truthy)`; a non-`true` return aborts the transaction with `:transact/attr-pred`. Enforced on assertion only (never on retract) under **both** `:read` and `:write` schema-flexibility, gated on an O(1) rschema lookup so unconstrained attributes pay nothing, and addable to already-defined attributes. Under `:attribute-refs? true` the constraint attrs get stable system entity IDs (like `:db.secondary/*`); a pre-existing attribute-refs store needs an ident-ref-map migration before it can use them. *Experimental.* ([#861])
- **Default value-size resource model** — newly created databases get Datomic-parity default caps so oversized values can't silently bloat the index: `:db.type/string` → 4096 chars, `:db.type/bytes` → 4096 bytes, and string slots inside a `:db.type/tuple` → 256 chars. Configurable at creation (`:max-string-length` / `:max-bytes-length` / `:max-tuple-string-length`; `0` disables) and overridable per attribute (explicit `:db/maxLength`). The default applies only under `:write` (it keys off the declared type) and to user attributes; `:db.secondary/only` values are exempt. **Existing databases are unaffected** — the caps live in the persisted config, and a store created before this feature has none, so it stays unbounded. Store large payloads off-database as a pointer or in a secondary index. *Experimental.* ([#861])
