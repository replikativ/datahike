...
- **Incremental view maintenance — standing query subscriptions (`datahike.ivm`)** — `subscribe!` registers a query on a connection and maintains its result across transactions instead of re-running it: conjunctive queries (patterns, predicates, pure function binds, `:in` scalars — with wildcards and constant positions) are maintained **incrementally** by per-clause telescoped delta queries — the transaction's datom batch, carrying Z-set weights (+1 add / −1 retract), is bound as an input relation and joined against the two commit snapshots through the ordinary query engine, so maintenance cost scales with the delta, not the database. Per-view state is a tuple-multiplicity map; the callback receives set-level changes only on zero crossings (`{:tx t :db-after db :changes [[tuple ±1] …]}` after an initial `{:reset true :result …}` snapshot), and the folded state equals `(q query db-after)` at every delivered tx — the invariant a seeded generative ratchet asserts after every transaction. Queries outside the class (aggregates, pull, or/not/rules, db-reading functions, non-scalar inputs) degrade transparently to re-evaluation + diff: same change stream, no incrementality (`strategy` reports which you got). Maintenance self-heals through an epoch reset on any failure or watermark gap. Works on both platforms; on cljs `:sync? false` runs every internal query through the async engine, serialized per subscription — reactive UIs over IndexedDB/remote stores subscribe instead of polling. *Experimental.* ([#TODO])
- **Asynchronous reads and transactions on ClojureScript (`:sync? false`)** — the full engine now runs against stores that cannot serve a single synchronous read — browser clients over IndexedDB/S3 — with **zero warmup** and no API duplication: `q`, `pull`, `pull-many`, `datoms`, `seek-datoms`, `rseek-datoms`, `index-range` and `with` accept `:sync? false` in their existing arg-map forms (the same map that carries `:offset`/`:limit` for `q`) and return a partial-cps async expression — invoke it with `(expr resolve reject)` or await it in an async block; connection-level `transact!` runs the same pipeline through the writer. Every index read *and write* is genuinely awaited — planning (the estimator records, gathers concurrently, re-plans pure), all find specs, predicates and the db-reading builtins, rules incl. mutual/recursive fixpoints, temporal/filtered/nested wrapper stacks, pull (wildcard + recursion), lookup refs and Date time-points, and on the transaction side upserts, cas, unique checks and retraction cascades — tree-rebalancing sibling loads included. Warm stores resolve **on the calling stack** (all-warm async ≡ sync, continuously pinned), so async is not a slow mode: it degrades to suspension only where a read is actually cold. Correctness never depends on cache residency — a read the store cannot serve fails cleanly (`:storage/sync-read-unavailable`), never wrong data. The JVM is untouched by construction: the dual bodies compile to the previous synchronous code (virtual threads make an async engine unnecessary there). Remaining sync islands (query-stats, collection-only sources, db-reading filter predicates such as `valid-at`'s, tx-function bodies, purge) surface the clean fault. Pinned by two seeded generative differentials replayed sync / async-warm / async-cold with exact coverage ratchets: queries 300/300, transactions at their ceiling (163/200; the rest raise identically in every mode). *Experimental.* ([#TODO])
- **`d/filter` and composed wrapper stacks join the query planner — on both platforms** — filtered databases were the largest whole-query fallback to the legacy engine, and nested temporal wrappers (`(d/history (d/as-of db t))`, `d/valid-at`, filtered-over-temporal) fell back entirely; the gates were shared cljc, so the JVM had the same limitations. Filtered queries now get planner join ordering *and* fused scans (the kernels apply the visibility predicate at scan and merge level — a filtered merge datom counts as absent, so anti-merges pass and `get-else` defaults fire), and arbitrary wrapper stacks fold into a normal form the fused path executes: `as-of` composes by minimum, `since` by maximum, mixed bounds form a tx window, history absorbs assembly regardless of nesting order, and the filter predicate commutes — the algebra mirrors the legacy context machinery exactly, validated against it by a 3000-case generative differential per build. ([#TODO])
...
- **Cardinality-one upserts now report the displaced datom as a retraction in `:tx-data`** — replacing a cardinality-one value dropped the old datom from the indices *silently*: the tx-report carried only the add, so `db-before`+`tx-data` did not reproduce `db-after` and every listener-driven consumer (sync, caches, IVM) was blind to the removal. The upsert path already knew the displaced datom; it is now reported as `[e a v-old tx false]` before the add — Datomic/DataScript parity, making `:tx-data` a complete delta of the current index. (`:db.secondary/only` attributes are excepted: their primary datom holds only the content hash, so no honest retraction value exists.) ([#TODO])
- **A throwing listener callback can no longer block the transact caller forever** — listener callbacks ran between the writer's commit acknowledgement and the delivery of the caller's promise, so one throwing listener left `transact` blocked indefinitely. Callbacks are now individually caught and logged (as the kabel writer already did). ([#TODO])
- **Cartesian-split: queries with `:in` relation bindings no longer NPE** — the disjoint-component split ignored two things about input bindings: a bound relation *joins* components through its columns, and it (or an `:in` scalar) can supply `:find` variables no component produces — the wide-tuple projection then indexed with `nil`. Such queries now skip the split and run through the relational pipeline, which handles them correctly. `:disable-planner?` passed on the query arg-map is also honored now (it was silently overwritten by the dynamic-var snapshot). ([#TODO])
- **Temporal queries with numeric time-points no longer probe `:db/txInstant` per transaction** — `as-of`/`since` filtering fetched the txInstant datom of every distinct tx in a result set even when the time-point was a tx id, where the predicate reduces to a comparison on the datom's own tx. The search context now carries the pure tx predicate alongside the general one (Date-based time-points keep the txInstant filter), removing N index probes per temporal read — and making temporal post-processing pure, which the async cold path depends on. ([#TODO])
- **Query cancellation was silently swallowed on the planner's fallback paths** — `dt/rethrowable?` tested an ex-data shape (`{:type :datahike/canceled}`) that the engine's cancellation check never produced (`{:datahike/canceled true}`), so broad catch sites — the fused-relation probe, planner fallbacks — caught a cancellation and *re-executed* the query instead of aborting: exactly the uncancellable query the predicate exists to prevent. Both shapes are now recognized; an async-mode cancellation test pins the behavior. ([#TODO])
