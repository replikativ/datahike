...
- **Re-asserting an unchanged value no longer appears in `:tx-data`** — transacting `[:db/add e a v]` a second time, for a value the entity already holds (cardinality-one) or already carries among its values (cardinality-many), still emitted that datom into the transaction report's `:tx-data`, so two identical back-to-back transactions no longer produced an empty data diff — the property that lets `:tx-data` be read as *what actually changed*. DataScript's skip-when-unchanged guard had been dropped during the mid-2020 upsert refactor (`e9d55972`), which re-emitted the datom unconditionally ever since — a regression from 0.2.1, reported in [#457]. The transaction reporter now omits the datom from `:tx-data` when the exact `[e a v]` is already live, comparing through the index's own datom comparator so it is correct for `:db.type/bytes` and tuple values (where Clojure `=` would compare byte arrays by identity). Only the `:tx-data` entry is suppressed — the write itself still runs, so secondary indices, history, and composite-tuple materialization are unaffected; a genuine change still asserts, cardinality-one still replaces the prior value, and retracting an absent value is unchanged. ([#457], [#894])
...
[#457]: https://github.com/replikativ/datahike/issues/457
[#894]: https://github.com/replikativ/datahike/pull/894
