...
- **cljs async foundation — the storage seam speaks async, and the synchronous q API fails honestly** — groundwork for browser clients querying remote backends (IndexedDB, S3). The index storage layer (`CachedStorage.restore`) now honors `{:sync? false}`: a datahike-LRU hit resolves on the calling stack (partial-cps trampolining — all-warm async computations complete *synchronously*, the property the upcoming async engine's sync mode rests on); a miss over a sync-capable store takes an opportunistic synchronous read and stays on the trampoline; only genuinely async-only stores pay the channel hop, where real IO dominates anyway. Store capability is determined **empirically** — one probed sync read, memoized per store — rather than from a hand-maintained backend taxonomy that could drift. The synchronous cljs `q` now checks that capability at dispatch and raises an actionable error on async-only stores (previously: a konserve assertion from deep inside index-node loading), deliberately gated on the *store*, not on cache temperature — a query that only sometimes completes synchronously is the flaky behavior the async API exists for. DataScript-style synchronous use over memory / pre-synced tiered stores is unchanged. *Experimental.* ([#TODO])
