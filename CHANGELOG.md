...
- **RDF / RDF-star on Datahike (`datahike.experimental.rdf`)** — the semantic-web node kinds mapped onto the datom model: an IRI is a `:db/unique` entity, a blank node an eid, a **literal** an **interned entity** keyed by its canonical term, and a **quoted triple** a reified statement. Literals are interned (Virtuoso's `RO_ID` model) rather than given a native value type, because RDF *term* equality (lexical ∧ datatype ∧ lang — what the index, joins, `DISTINCT` and `COUNT` key on) is not *value* equality (`"1"^^xsd:integer` = `"01"^^xsd:integer`, computed at query time): a value type would have to make distinct terms compare equal to sort numerically, collapsing them in the index. Interning makes that impossible, and numeric **range** stays index-native via a `:literal/num` (`:db.type/bigdec`) shadow that `d/index-range` seeks — with no RDF code in the core comparator. Term identity canonicalizes lang tags to lower-case and plain literals to `xsd:string`, preserving the lexical form verbatim. RDF-star `annotate` asserts the ground triple *and* reifies it (addressable by a static lookup-ref); `quote-triple` reifies only. `rdf-term-compare` gives the SPARQL ORDER BY total order over mixed terms. **Named graphs are a federation of stores** (`db-id` ≙ graph) — a deliberate 1.0 scope: it fits graph-scoped datasets but is not the single-dataset-many-graphs SPARQL model, and a quad (4th datom position) is a deferred storage-format decision. See [RDF and RDF-star](doc/rdf.md). *Experimental.* ([#864])
...
[#864]: https://github.com/replikativ/datahike/pull/864
