...
- **GC clock correctness — write stamps and collection cutoffs share one monotonic source** — the sweep's safety argument ("every object a guarded write sequence produces carries a `last-write` ≥ the sequence's safe-point") compared timestamps from **two independent wall-clock reads** (datahike's gc-guard and konserve's write stamping) and silently assumed machine clocks are monotonic. They are not: an NTP step-back, VM suspend/resume or manual clock set inside the guard window could stamp a *live* mid-flush object before its cutoff, and the sweep would delete it — dangling pointers on the next cold read. konserve's `:last-write` is now a **hybrid logical clock** (`max(wall-clock, prev + 1ms)`, strictly increasing, still a readable `Date`), and every collector-side clock read — the gc-guard safe-point, `gc-storage!`'s cutoff, background-GC's retention boundary, online-GC's freed-address stamps and grace cutoff — reads the *same source*, making happens-before literal in the stamps instead of an assumption. Strict monotonicity also kills the same-millisecond tie class outright: the store-ref GC test's settling-delay workaround is removed and survives repeated stress runs. Requires konserve ≥ TBD (replikativ/konserve#156). Single-process writers+collector, as before ([#879]). ([#TODO])
