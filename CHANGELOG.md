...
- **Robust export/import (`datahike.migrate`)** — the 3-arity `export-db`/`import-db` produce and consume a *type-exact*, verifiable EDN-lines dump (`doc/import-export-design.md`) that round-trips what the old flat CBOR snapshot could not: full history (assertions, retractions, tx entities incl. `:db/txInstant`, so `history`/`as-of`/`since` match at every `t`, [#377]), schema-before-data replay ([#262]), attribute-refs databases without re-inserting system datoms — refs to system entities are *translated* by ident, not inserted ([#508]/[#531]), and every builtin value type exactly ([#633], below). Import runs through `load-entities` (entity/tx ids are remapped, so a restore is *semantically* equal, not id-identical), refuses a non-empty target (import is not resumable — the id-map is memory-only, so recreate-and-restart), and self-verifies (per-chunk SHA-256 + an order-independent semantic digest; `verify` and `finalize-import!` are public). A `:chunked` directory format streams to `datoms-NNNNNN.edn` files with a `manifest.edn` commit-marker; the `:flat` single-file format and the legacy 2-arity (which still *reads* old CBOR dumps) remain. Parsing is `clojure.edn` with a **closed** reader map (no `read-string`, no eval). **A `:history? true` dump resurrects retracted data** — use `:history? false` for data leaving a trust boundary. *Experimental.* ([#909])
...
- **`:db.type/double` survives export/import as a `Double`, not a `Float`** — the old CBOR dump codec routed zero/NaN/±Inf doubles through half-precision (clj-cbor `write-float`) and decoded them back with `Float/intBitsToFloat`, so a `:db.type/double` value like `0.0` returned as a `java.lang.Float`; datahike's schema is a bare `double?`/`float?` predicate with no coercion, so the flipped class fails validation and compares unequal. The new EDN-lines codec encodes by runtime class with type tags (`#datahike/float`, `#datahike/bytes`, `#datahike/farray`/`#datahike/darray`, `#datahike/sysref`), round-tripping `double`/`float`/`float-array`/`double-array`/`bytes`/`bigint`/`bigdec`/`symbol`/`uuid`/`instant`/`tuple` class-exactly. Only reachable through the new `datahike.migrate` export/import path. ([#633], [#909])
...
[#909]: https://github.com/replikativ/datahike/pull/909
[#897]: https://github.com/replikativ/datahike/issues/897
[#633]: https://github.com/replikativ/datahike/issues/633
[#531]: https://github.com/replikativ/datahike/issues/531
[#508]: https://github.com/replikativ/datahike/issues/508
[#377]: https://github.com/replikativ/datahike/issues/377
[#262]: https://github.com/replikativ/datahike/issues/262
